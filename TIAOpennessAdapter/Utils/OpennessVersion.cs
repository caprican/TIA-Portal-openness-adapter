...
namespace TIAOpennessAdapter.Utils
{
    ...
    public struct OpennessVersion
    {
        ...
        {
            if (tiaPortalVersion is null || tiaPortalVersion.Major == 0 &&
                tiaPortalVersion.Minor == 0 &&
                tiaPortalVersion.Build == 0 &&
                tiaPortalVersion.Revision == 0)
            {
            ...
            {
                throw new ArgumentNullException(nameof(libraryFilePath));
            }

            if (publicApiVersion is null || publicApiVersion.Major == 0 &&
                publicApiVersion.Minor == 0 &&
                publicApiVersion.Build == 0 &&
                publicApiVersion.Revision == 0)
            {
